...
def run_graph2vec(args):
    ...
    for patient in args.patients:
        all_graphs.append(f"{os.path.join(args.input_path, patient)}.json")
    # graphs = glob.glob(os.path.join(args.input_path, "*.json"))
    print("\nFeature extraction started.\n")
    document_collections = Parallel(n_jobs=args.workers)(
        delayed(feature_extractor)(g, args.wl_iterations) for g in tqdm(all_graphs))
    ...
    save_embedding(args.output_path, model, all_graphs, args.dimensions)
